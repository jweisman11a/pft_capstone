...
def scrape_comments_from_pft_article(url='https://profootballtalk.nbcsports.com/2020/10/03/eagles-add-five-from-practice-squad-ahead-of-sundays-game/'):
    ...
    chrome_options.add_experimental_option('excludeSwitches', ['enable-logging'])  # prevents CLI logging which screws up tqdm
...
def scrape_comments_from_pft_article_v2(url='https://profootballtalk.nbcsports.com/2020/10/03/eagles-add-five-from-practice-squad-ahead-of-sundays-game/'):
    """Function scrapes comments from a single article on PFT
    ...
    try:
        ...
        # Compile results
        comment_page = list(zip([url for _ in range(len(commentors))], commentors, times,
                                comments, [dt.datetime.now() for _ in range(len(commentors))]))

        return pd.DataFrame(comment_page, columns=['article_url','commentor','comment_datetime',
                                                   'comment_body','scrape_datetime'])

    except:

        return pd.DataFrame(columns=['article_url','commentor','comment_datetime',
                                     'comment_body','scrape_datetime'])
